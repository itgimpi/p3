...
typedef pair<int, int> tacka; // O(q log n)
...
bool pripada( const vector<tacka> &mnogo, const tacka &a, int l, int d ) {
    // DC solution
    if ( d - l == 1 ) // BC, samo 1 trougao
        return utrouglu(mnogo[0], mnogo[l], mnogo[d], a);

    int s = l + (d-l)/2; // s = (l+d)/2
    if ( ori( mnogo[0], mnogo[s], a ) == 1 )
        return pripada(mnogo, a, s, d);
    else    
        return pripada(mnogo, a, l, s);
