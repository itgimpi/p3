...
Kad se dodaje najc cvor skupu Vk, azuriraju se samo rastojanja susednog cvora, ostala se ne menjaju. */
...
int main() {
    ...
    while (br < n) {  // dok svi cvorovi ne zavrse u stablu...
        //int najc = 0;       // cvor koji je najblizi trenutnom stablu je inicijalno prvi
        int najc; // cvor koji je najblizi trenutnom stablu
        double najr = INF;   // pocetno rastojanje najrblizeg cvora od stabla
        for (int cvor = 0; cvor < n; cvor++) // za sve cvorove, nadji najblizi cvor stablu
            if ( !pripada[cvor] && rastojanje[cvor] < najr ) { // ako nije u stablu a blizi je
                najc = cvor;               // nadji min rastojanje
                najr = rastojanje[cvor]; } // i proglasi ga najmanjim

        pripada[najc] = true;    // najblizi stablu ide u stablo
        br++; // evidentira se
        res += najr; // resenje se poveca
        rastojanje[najc] = 0.0;  // sad pripada stablu i rastojanje od stabla je 0

        for ( auto v : veze[najc] )  // za sve susede novog clana stabla...
            if (v.first < rastojanje[v.second]) // ako je rastojanje suseda manje od 
                rastojanje[v.second] = v.first;     } // azurira se rastojanje od stabla
